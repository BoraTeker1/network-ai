...
def _get_or_404(db: Session, email_id: int) -> EmailDraft:
    e = db.query(EmailDraft).filter(EmailDraft.id == email_id).first()
    if e is None:
...
@router.post("/draft")
def draft_email(payload: EmailDraftIn, db: Session = Depends(get_db)):
...
def list_emails(db: Session = Depends(get_db)):
    emails = (
        db.query(EmailDraft)
        .filter(EmailDraft.user_id == DEMO_USER_ID)
...
def get_email(email_id: int, db: Session = Depends(get_db)):
    return _serialize(_get_or_404(db, email_id), db)


# ----- Edit -----

@router.patch("/{email_id}")
def patch_email(email_id: int, payload: EmailPatchIn, db: Session = Depends(get_db)):
    """Edit subject/body (recomputes quality checklist) or set outcome/follow-up."""
    e = _get_or_404(db, email_id)
...
def _set_status(db: Session, email_id: int, status: str) -> dict:
    e = _get_or_404(db, email_id)
    e.status = status
    db.commit()
    db.refresh(e)
...
def approve_email(email_id: int, db: Session = Depends(get_db)):
    return _set_status(db, email_id, "approved")


@router.post("/{email_id}/reject")
def reject_email(email_id: int, db: Session = Depends(get_db)):
    return _set_status(db, email_id, "rejected")


@router.post("/{email_id}/mark-copied")
def mark_copied(email_id: int, db: Session = Depends(get_db)):
    return _set_status(db, email_id, "copied")


@router.post("/{email_id}/mark-sent-manual")
def mark_sent_manual(email_id: int, db: Session = Depends(get_db)):
    return _set_status(db, email_id, "sent_manual")


# ----- Gmail send (disabled placeholder) -----

@router.post("/{email_id}/send-gmail")
def send_gmail(email_id: int, payload: GmailSendIn, db: Session = Depends(get_db)):
